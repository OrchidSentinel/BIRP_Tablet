...
-- ── Audio (xSound) ────────────────────────────────────────────────────────────
--  Inhaltsseiten spielen Ton im Browser nativ ab; ingame schicken sie NUI-Callbacks
--  an diese Resource. Damit UMSTEHENDE den Ton mithoeren, wird er NICHT lokal,
--  sondern serverseitig als 3D-Sound an der Spielerposition abgespielt
--  (server/server.lua -> xSound PlayUrlPos an alle Clients).
local function stopTape()
    TriggerServerEvent('BIRP_Tablet:tapeAudio', { action = 'stop' })
end
...
-- Wird an den Server gereicht, der den Ton fuer ALLE Clients raeumlich abspielt.
RegisterNUICallback('tapeAudio', function(data, cb)
    data = data or {}
    TriggerServerEvent('BIRP_Tablet:tapeAudio', {
        action = data.action,
        url    = data.url,
        time   = data.time,
        volume = data.volume,
